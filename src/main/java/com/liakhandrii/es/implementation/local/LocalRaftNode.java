...
public class LocalRaftNode extends NodeCore<String> {
    ...
    protected AppendEntriesResponse sendEntries(NodeAccessor nodeAccessor) {
        ...
        if (lastNodeEntry != null) {
            request = new AppendEntriesRequest<>(currentTerm, id, lastNodeIndex, lastNodeEntry.getTerm(), newEntries, commitIndex());
        } else {
            request = new AppendEntriesRequest<String>(currentTerm, id, -1, currentTerm, newEntries, commitIndex());
        }
